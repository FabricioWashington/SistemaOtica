...
import DTO.Cadastro.CadastroEmpresaDTO;
...
public class CadastroPessoaJuridicaDAO {
    ...
    public void cadastrarUsuarioCompleto(EnderecoDTO objenderecodto, CadastroPessoaJuridicaDTO objcadastropessoajuridicadto, ContatoDTO objcontatodto) {
        ...
        try {
            ...
            String sqlCadastroPessoaJuridica = "insert into cadastro_cliente_juridico (CNPJ, Razao_Social, Nome_Fantasia, idEndereco, idContato, Data_Cadastro, Data_Modificacao) values (?,?,?,?,?,?,?)";
            PreparedStatement pstmCadastroPessoaJuridica = conn.prepareStatement(sqlCadastroPessoaJuridica, Statement.RETURN_GENERATED_KEYS);
            pstmCadastroPessoaJuridica.setString(1, objcadastropessoajuridicadto.getCNPJ());
            pstmCadastroPessoaJuridica.setString(2, objcadastropessoajuridicadto.getRazao_Social());
            pstmCadastroPessoaJuridica.setString(3, objcadastropessoajuridicadto.getNome_Fantasia());
            pstmCadastroPessoaJuridica.setInt(4, idEndereco);
            pstmCadastroPessoaJuridica.setInt(5, idContato);
            pstmCadastroPessoaJuridica.setTimestamp(6, new java.sql.Timestamp(objcadastropessoajuridicadto.getData_Cadastro().getTime()));
            pstmCadastroPessoaJuridica.setTimestamp(7, new java.sql.Timestamp(objcadastropessoajuridicadto.getData_Modificacao().getTime()));
            pstmCadastroPessoaJuridica.executeUpdate(); // Use executeUpdate() para inserções
    ...
    public boolean verificarCadastroAdminExistente(CadastroEmpresaDTO objcadastroempresadto) {
        conn = new ConexaoDAO().conectaBD();
